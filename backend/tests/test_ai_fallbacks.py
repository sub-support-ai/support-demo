...
@pytest.mark.asyncio
async def test_get_ai_answer_records_connect_fallback(
    ...
    events = (
        await db_session.execute(
            select(AIFallbackEvent).where(
                AIFallbackEvent.conversation_id == conversation.id
            )
        )
    ).scalars().all()
    assert len(events) == 1
    assert events[0].service == "answer"
    assert events[0].reason == "connect"
...
async def test_get_ai_answer_records_broken_json_fallback(
    ...
    await db_session.flush()
    db_session.add(
        Message(conversation_id=conversation.id, role="user", content="anything")
    )
    await db_session.flush()
    ...
    class _StubClient:
        ...
        async def post(self, *args, **kwargs): return _OkResponse()
...
@pytest.mark.asyncio
async def test_classify_ticket_records_connect_fallback(
...
@pytest.mark.asyncio
async def test_fallbacks_endpoint_aggregates_by_reason_and_service(
    client: AsyncClient, db_session
):
...
@pytest.mark.asyncio
async def test_fallbacks_endpoint_respects_since_window(
    client: AsyncClient, db_session
):
...
@pytest.mark.asyncio
async def test_fallbacks_endpoint_clamps_since_to_max_window(
    client: AsyncClient, db_session
):
