...
class AILog(Base):
    ...
    routed_to_agent_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("agents.id", ondelete="SET NULL"), nullable=True
    )
    ai_response_draft: Mapped[str | None] = mapped_column(Text, nullable=True)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    routing_was_correct: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    agent_corrected_category: Mapped[str | None] = mapped_column(String(100), nullable=True)
    agent_accepted_ai_response: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    correction_lag_seconds: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # Время ответа AI Service в миллисекундах — питч-дек обещает "1,01 сек",
    # этот столбец даёт честную цифру из прода.
    ai_response_time_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # ──────────────────────────────────────────────────────────────────────────
